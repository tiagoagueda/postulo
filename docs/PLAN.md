...
| Extensibility | **A plugin interface wherever a choice could reasonably vary** | Capture sources are the first; notifications are next, with SMTP built in and an Apprise plugin as the first external one (see the 0.2.0 milestone). Postulo's own implementations are plugins that ship in the box. |
| Security | **As secure as the project knows how to make it** | The application holds the most personal documents a person has while looking for work. Ownership-scoped queries, permission-checked files, a strict CSP, no unrequested network calls, security tests that fail the build, and dependency vulnerability checks on every run and on a schedule, so a new disclosure is noticed. A security report comes before any feature. |
| Accessibility | **Usable by everyone, at its fullest, including people with disabilities** | Server-rendered HTML that works with scripts off; keyboard-operable; names on icons or `aria-hidden`; colour never the only carrier of meaning; live regions for changes; WCAG 2.2 AA checked in the browser tests. A feature somebody cannot use is a bug. |
