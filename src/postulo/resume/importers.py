...
produce the same :class:`~postulo.resume.europass.Record`, `apply()` is shared, and `read()`
already tells them apart. Two plugins would be one thing described twice.

``apply()`` stays in core and deliberately: an importer turns bytes into a record and does
not touch the database. That is what keeps an importer — including one somebody else writes
— from needing ownership scoping of its own.
"""
